...
def init_db():
    ...
    with get_connection() as conn:
        ...
        # Call analytics table — stores one row per completed call
        # NEVER stores transcripts, OTP, PIN, CVV, password, or account numbers
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS call_analytics (
                call_id        TEXT PRIMARY KEY,
                started_at     TEXT NOT NULL,
                ended_at       TEXT,
                outcome        TEXT CHECK(outcome IN ('SUCCESS', 'FAILED')) DEFAULT 'FAILED',
...
def get_analytics_stats() -> Dict[str, Any]:
    ...
    with get_connection() as conn:
        ...
        cursor.execute("SELECT COUNT(*) as total FROM call_analytics WHERE outcome = 'SUCCESS'")
        successful_calls = cursor.fetchone()["total"]

        cursor.execute("SELECT COUNT(*) as total FROM call_analytics WHERE outcome = 'FAILED'")
        failed_calls = cursor.fetchone()["total"]

        success_rate = (
            round((successful_calls / total_calls) * 100, 1) if total_calls > 0 else None
        )
...
def record_call_analytics(
    ...
    """Record outcome analytics for a completed call.

    Stores only: call_id, started_at, ended_at, outcome (SUCCESS/FAILED), outcome_reason.
    NEVER stores OTP, PIN, CVV, password, account number, or any transcript content.

    Args:
        call_id: Unique call/session identifier (room name)
        started_at: ISO timestamp when the call started
        ended_at: ISO timestamp when the call ended
        outcome: 'SUCCESS' or 'FAILED'
        outcome_reason: Short human-readable reason (sanitized before storage)
    """
    init_db()

    # Sanitize outcome_reason to guarantee no sensitive data leaks in
    safe_reason = sanitize_sensitive_data(outcome_reason.strip() if outcome_reason else "")

    # Normalise outcome to the two allowed values
    normalised_outcome = "SUCCESS" if str(outcome).upper() == "SUCCESS" else "FAILED"

    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
            INSERT INTO call_analytics (call_id, started_at, ended_at, outcome, outcome_reason)
            VALUES (?, ?, ?, ?, ?)
            ...
            """,
            (call_id, started_at, ended_at, normalised_outcome, safe_reason),
        )
        conn.commit()
...
def get_call_analytics(limit: int = 100) -> List[Dict[str, Any]]:
    """Return recent call analytics rows ordered newest-first."""
    init_db()
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
